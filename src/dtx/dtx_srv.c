 ...
 * current known the highest HLC. In theory, it should be the highest one
 * across the whole system, any transaction with old epoch (HLC) in spite
 * of being generated by which server will be reguarded as started before
...
dtx_handler(crt_rpc_t *rpc)
{
	...
	case DTX_CHECK:
		 ...
		 * modification with old eppch, that have the same effect
