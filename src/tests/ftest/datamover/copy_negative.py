...
class CopyNegativeTest(IorTestBase):
    ...
    def extract_rc(self, s):
        ...
        rc_search = re.search("\(rc=([0-9]+)\)", str(s))
        if not rc_search:
            return None
        else:
    ...
    def run_dcp(self, source, target,
                ...
                test_desc=None, expected_error_code=None):
        ...
        # TODO fix this once DataMover has been updated
        # Will need to always override both to account for default values
        if override_src_svcl is not None:
            dcp.daos_svcl.update_default(None)
            dcp.daos_svcl.update(self.nvl(override_src_svcl))
        if override_dst_svcl is not None:
            dcp.daos_svcl.update_default(None)
            dcp.daos_svcl.update(self.nvl(override_dst_svcl))
