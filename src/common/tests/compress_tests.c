...
test_alg_basic(const char *alg_name)
{
	...
	int compr_output_sz = 0;
	int decompr_output_sz = 0;
	...
	/** Perform compress */
	compr_output_sz = daos_compressor_compress(
				compressor,
				origin_buf,
				origion_sz,
				comp_buf,
	...
	if (verbose)
		print_message("%s: compressed %d bytes --> %d bytes.\n",
			      alg_name, origion_sz, compr_output_sz);

	/** Perform decompress */
	decompr_output_sz = daos_compressor_decompress(
				compressor,
				comp_buf,
				compr_output_sz,
				decomp_buf,
	...
	if (verbose)
		print_message("%s: decompressed %d bytes --> %d bytes.\n",
			      alg_name, compr_output_sz, decompr_output_sz);
