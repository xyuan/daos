			  ...
			  daos_recx_t *reply_recx)
{
	...
	uint64_t		 stripe_rec_nr, cell_rec_nr, rx_idx, rx_end;
	...
	tgt_idx = cb_args->shard->do_pl_shard.po_shard %
		  obj_ec_tgt_nr(oca);
	...
	rx_end = reply_recx->rx_idx + reply_recx->rx_nr;
	rx_idx = max(reply_recx->rx_idx, rounddown(rx_end - 1, cell_rec_nr));
	result_recx->rx_nr = rx_end - rx_idx;
	rx_idx = obj_ec_idx_vos2daos(rx_idx, stripe_rec_nr, cell_rec_nr,
				     tgt_idx);
	result_recx->rx_idx = rx_idx;
}
