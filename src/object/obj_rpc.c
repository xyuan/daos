...
static int
crt_proc_internal_dtx_redundancy_group(crt_proc_t proc,
				       struct dtx_redundancy_group *p)
{
	int	rc;

	rc = crt_proc_uint32_t(proc, &p->drg_tgt_cnt);
	if (unlikely(rc))
		return rc;

	rc = crt_proc_uint32_t(proc, &p->drg_redundancy);
	if (unlikely(rc))
		return rc;

	if (p->drg_tgt_cnt > 0) {
		rc = crt_proc_memcpy(proc, p->drg_ids,
				     p->drg_tgt_cnt * sizeof(p->drg_ids[0]));
		if (unlikely(rc))
			return rc;
	}

	return sizeof(*p) + sizeof(p->drg_ids[0]) * p->drg_tgt_cnt;
}
...
crt_proc_struct_dtx_memberships(crt_proc_t proc, struct dtx_memberships *mbs)
{
	...
	if (mbs->dm_grp_cnt > 0) {
		...
		for (i = 0; i < mbs->dm_grp_cnt; i++) {
			...
			rc = crt_proc_internal_dtx_redundancy_group(proc, drg);
			if (unlikely(rc < 0))
				return rc;
		}
	}
