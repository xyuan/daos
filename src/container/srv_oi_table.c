...
static int
cont_sent_oit_bucket(struct oit_scan_args *oa, uint32_t bucket_id)
{
	 ...
	 * the same object ID can be overwritten for many times in different
	 * epochs and consume way more space.
	 */
		 ...
		 vos_iter_param_t *param, void *data, unsigned *acts)
{
	...
	rc = cont_sent_oit_bucket(oa, bid);
